...
import { hasRole, Role } from '@/config/permissionCore';
...
let projectServicePromise: Promise<typeof import('@/services/projectService')> | null = null;
let customerServicePromise: Promise<typeof import('@/services/customerService')> | null = null;
let userServicePromise: Promise<typeof import('@/services/userService')> | null = null;

const loadProjectService = async () => {
  projectServicePromise ??= import('@/services/projectService');
  return projectServicePromise;
};

const loadCustomerService = async () => {
  customerServicePromise ??= import('@/services/customerService');
...
export default function AddProject() {
  ...
  useEffect(() => {
    ...
    void Promise.all([loadCustomerService(), loadUserService()])
      .then(async ([customerService, userService]) => {
        const [customerData, userData] = await Promise.all([
          customerService.getAllCustomers(),
          userService.getAllUsers(),
        ]);

        const clients = customerData.clients || [];
      ...
      .catch((error) => {
        logger.error('Proje form verileri yüklenirken hata:', error);
        toast.error('Proje form verileri yüklenirken bir hata oluştu');
      });
  }, []);
