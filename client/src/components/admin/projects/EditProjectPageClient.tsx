...
function EditProjectContent() {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      try {
        ...
        const [projectService, customerService, userService] = await Promise.all([
          loadProjectService(),
          loadCustomerService(),
          loadUserService(),
        ]);
        ...
        const [project, customerRes, userRes] = await Promise.all([
          ...
          userService.getAllUsers({ page: 1, limit: 1000 }),
        ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const { updateProject } = await loadProjectService();
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 space-y-6">
        ...
        <div className="flex justify-end items-center space-x-4 pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <Link href={`/admin/projects/view?id=${projectId}`}>
            <button
              type="button"
