...
const loadProjectService = (() => {
  let promise: Promise<typeof import('@/services/projectService')> | null = null;
  return () => {
    promise ??= import('@/services/projectService');
    return promise;
  };
})();

const loadMaintenanceService = (() => {
  let promise: Promise<typeof import('@/services/maintenanceService')> | null = null;
  return () => {
    promise ??= import('@/services/maintenanceService');
    return promise;
  };
})();
  ...
  const handleDropOnDate = useCallback(
    async (e: React.DragEvent, targetDate: Date) => {
      ...
      if (payload.type === 'project') {
        ...
        try {
          const { updateMaintenance } = await loadMaintenanceService();
