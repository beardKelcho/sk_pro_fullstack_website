...
const typeColors = {
  'Periyodik Bakım': 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-400',
  'Arıza Bakımı': 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400',
...
export default function MaintenanceList() {
  ...
  const getEquipmentName = (item: Maintenance) => {
    const extendedItem = item as Maintenance & { equipmentName?: string };
    const equipmentValue = (item as Maintenance & { equipment?: unknown }).equipment;

    if (equipmentValue && typeof equipmentValue === 'object') {
      const equipmentWithName = equipmentValue as { name?: string };
      return equipmentWithName.name || extendedItem.equipmentName || '';
    }

    return extendedItem.equipmentName || '';
  };

  const getPriorityValue = (item: Maintenance) => {
    const extendedItem = item as Maintenance & { priority?: string };
    return extendedItem.priority || '';
  };

  const getAssignedToName = (item: Maintenance) => {
    if (typeof item.assignedTo === 'object' && item.assignedTo) {
      return item.assignedTo.name || '';
    }
    return '';
  };

  const getStatusValue = (item: Maintenance) => mapBackendMaintenanceStatusToFrontend(item.status);
  const getPriorityLabelValue = (item: Maintenance) => mapBackendMaintenancePriorityToFrontend(getPriorityValue(item));
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [debouncedSearchTerm, selectedType, selectedStatus, selectedPriority]);
    ...
    const fetchMaintenance = async () => {
      ...
      try {
        const data = await getAllMaintenance({
          ...
          type: selectedType !== 'Tümü' ? mapFrontendMaintenanceTypeToBackend(selectedType) : undefined,
          status: selectedStatus !== 'Tümü' ? mapFrontendMaintenanceStatusToBackend(selectedStatus) : undefined,
          priority: selectedPriority !== 'Tümü' ? mapFrontendMaintenancePriorityToBackend(selectedPriority) : undefined,
        });
  ...
  }, [currentPage, debouncedSearchTerm, selectedType, selectedStatus, selectedPriority]);
  ...
  const handleDeleteMaintenance = async () => {
    ...
    setIsDeleting(true);
    try {
      const shouldMoveToPreviousPage = maintenance.length === 1 && currentPage > 1;
      await deleteMaintenance(maintenanceToDelete);

      if (shouldMoveToPreviousPage) {
        setCurrentPage(prevPage => prevPage - 1);
      } else {
        setMaintenance(prevMaintenance => prevMaintenance.filter(m => (m.id || m._id) !== maintenanceToDelete));
      }

      setTotalMaintenance(prevTotal => Math.max(prevTotal - 1, 0));
  ...
  const activeFilterItems = [
    ...
    selectedType !== 'Tümü'
      ? {
          ...
          onRemove: () => setSelectedType('Tümü'),
        }
      : null,
    selectedStatus !== 'Tümü'
      ? {
          key: 'status',
          label: `Durum: ${selectedStatus}`,
          onRemove: () => setSelectedStatus('Tümü'),
        }
      : null,
    selectedPriority !== 'Tümü'
      ? {
          ...
          onRemove: () => setSelectedPriority('Tümü'),
        }
      : null,
    ...
    <div className="space-y-6">
      ...
      {hasActiveFilters && (
        <ActiveFiltersBar
          ...
          totalCount={totalMaintenance}
          visibleCount={maintenance.length}
          itemLabel="bakım kaydı"
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden">
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {maintenance.map(item => {
                  const itemAny = item as Maintenance & { startDate?: string; endDate?: string; assignedToName?: string };
                  const equipmentName = getEquipmentName(item);
                  const typeLabel = getTypeLabel(item.type);
                  const priorityLabel = getPriorityLabelValue(item);
                  const statusLabel = getStatusValue(item);
                    ...
                    <tr key={item.id || item._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col gap-1.5">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${typeColors[typeLabel as keyof typeof typeColors] || ''}`}>
                            {typeLabel}
                          </span>
                          {priorityLabel && (
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${priorityColors[priorityLabel as keyof typeof priorityColors] || ''}`}>
                              {priorityLabel}
                            </span>
                          )}
                        </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColors[statusLabel as keyof typeof statusColors] || ''}`}>
                          {statusLabel}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                        {itemAny.assignedToName || getAssignedToName(item) || (typeof item.assignedTo === 'string' ? item.assignedTo : '')}
                      </td>
        ...
        {!loading && maintenance.length > 0 && (
          <div className="bg-gray-50 dark:bg-gray-700/50 px-6 py-3 border-t border-gray-200 dark:border-gray-700 flex flex-col gap-3 md:flex-row md:items-center md:justify-between text-xs text-gray-500 dark:text-gray-400">
            <span>Gösterilen: {maintenance.length} / Toplam: {totalMaintenance}</span>
