...
import { Permission, rolePermissions, Role } from '@/config/permissionCore';
import {
  loadPermissionCatalog,
  type PermissionCatalogByCategory,
} from '@/config/permissionCatalog';

const PermissionAssignmentPanel = dynamic(
  () => import('@/components/admin/permissions/PermissionAssignmentPanel'),
  {
    loading: () => (
      <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
        <div className="animate-pulse space-y-4">
          ...
          <div className="space-y-3">
            <div className="h-24 rounded-lg bg-gray-100 dark:bg-gray-900/40" />
            <div className="h-24 rounded-lg bg-gray-100 dark:bg-gray-900/40" />
            <div className="h-24 rounded-lg bg-gray-100 dark:bg-gray-900/40" />
          </div>
        </div>
...
const PermissionDetailsPanel = dynamic(
  () => import('@/components/admin/permissions/PermissionDetailsPanel'),
  {
    loading: () => (
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
        <div className="animate-pulse space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="h-28 rounded-lg bg-gray-100 dark:bg-gray-900/40" />
            <div className="h-28 rounded-lg bg-gray-100 dark:bg-gray-900/40" />
            <div className="h-28 rounded-lg bg-gray-100 dark:bg-gray-900/40" />
            <div className="h-28 rounded-lg bg-gray-100 dark:bg-gray-900/40" />
          </div>
...
export default function PermissionsPage() {
  ...
  const [permissionsByCategory, setPermissionsByCategory] = useState<PermissionCatalogByCategory | null>(null);
  ...
  const ensurePermissionCatalog = async () => {
    ...
    try {
      const nextCatalog = await loadPermissionCatalog();
      setPermissionsByCategory(nextCatalog);
      return nextCatalog;
    } catch (error) {
      logger.error('Yetki kataloğu yükleme hatası:', error);
      toast.error('Yetki detayları yüklenirken bir hata oluştu');
      return null;
    } finally {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const { getAllUsers, mapBackendRoleToFrontend } = await loadUserService();
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'assign' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {selectedUser && permissionsByCategory && (
            <PermissionAssignmentPanel
              categories={permissionsByCategory}
              selectedUserName={selectedUser.name}
              userPermissions={userPermissions}
              isPermissionFromRole={isPermissionFromRole}
              onToggle={handlePermissionToggle}
            />
          )}

          {selectedUser && !permissionsByCategory && catalogLoading && (
            <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
              <div className="animate-pulse space-y-4">
                <div className="h-6 w-56 rounded bg-gray-200 dark:bg-gray-700" />
                <div className="h-24 rounded-lg bg-gray-100 dark:bg-gray-900/40" />
      ...
      {activeTab === 'details' && (
        <>
          {permissionsByCategory ? (
            <PermissionDetailsPanel categories={permissionsByCategory} />
          ) : (
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
              <div className="animate-pulse space-y-4">
                <div className="h-6 w-72 rounded bg-gray-200 dark:bg-gray-700" />
